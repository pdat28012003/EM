...
const Schedules = () => {
  ...
  const loadSchedule = async (studentId) => {
    try {
      ...
      const scheduleData = Array.isArray(response.data?.data) ? response.data.data : [];
      setSchedule(scheduleData);
      setTotalSchedule(response.data?.totalCount || scheduleData.length);
    } catch (err) {
        ...
        <Grid container spacing={2} alignItems="center">
          <Grid item xs={12} md={6}>
            <Autocomplete
              ...
              onInputChange={handleStudentsSearch}
