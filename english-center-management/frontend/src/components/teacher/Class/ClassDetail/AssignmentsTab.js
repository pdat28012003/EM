  ...
  export default function AssignmentsTab({ curriculumId, curriculumInfo }) {
    ...
    const { initialLoading, loading, startLoading, stopLoading } = useAsyncLoading();
    ...
    }, [curriculumId, page, rowsPerPage, statusFilter, typeFilter, skillFilter, sortBy, searchQuery, initialLoading, startLoading, stopLoading]);
    ...
    useEffect(() => {
      ...
      loadSkills();
    }, [curriculumId, page, rowsPerPage, statusFilter, typeFilter, skillFilter, sortBy, searchQuery, loadAssignments, loadSkills]);
    ...
    // assignmentCounts tính bằng useMemo để tránh recalculate mỗi render
    ...
    const handleSaveAssignment = async () => {
      ...
      try {
        ...
        // Trigger notification reload after creating assignment
      ...
      <Box>
        ...
        <Box display="grid" gridTemplateColumns="repeat(3, 1fr)" gap={2} mb={3}>
          <Card sx={{ borderRadius: 3, p: 2, display: 'flex', alignItems: 'center', gap: 2 }}>
            <Box sx={{ bgcolor: 'primary.main', borderRadius: 2, p: 1.5, color: 'white', display: 'flex' }}>
              <Assignment />
            </Box>
            <Box>
          ...
          <Card sx={{ borderRadius: 3, p: 2, display: 'flex', alignItems: 'center', gap: 2 }}>
            <Box sx={{ bgcolor: 'success.main', borderRadius: 2, p: 1.5, color: 'white', display: 'flex' }}>
              <Publish />
            </Box>
            <Box>
              <Typography variant="subtitle2" color="text.secondary" gutterBottom>
          ...
          <Card sx={{ borderRadius: 3, p: 2, display: 'flex', alignItems: 'center', gap: 2 }}>
            <Box sx={{ bgcolor: 'error.main', borderRadius: 2, p: 1.5, color: 'white', display: 'flex' }}>
              <LockOutlined />
            </Box>
            <Box>
              <Typography variant="subtitle2" color="text.secondary" gutterBottom>
        ...
        <Box mb={3}>
          <Box display="flex" flexWrap="wrap" gap={2} mb={2}>
            ...
            <FormControl size="small" sx={{ minWidth: 140 }}>
              ...
              >
                <MenuItem value="all">Tất cả</MenuItem>
                <MenuItem value="Essay">Bài tập về nhà</MenuItem>
            ...
            <FormControl size="small" sx={{ minWidth: 140 }}>
              ...
              >
                <MenuItem value="all">Tất cả</MenuItem>
                {skills.map((skill) => (
                  <MenuItem key={skill.skillId || skill.SkillId} value={skill.skillId || skill.SkillId}>
            ...
            <FormControl size="small" sx={{ minWidth: 140 }}>
              ...
              >
                <MenuItem value="dueDate">Hạn nộp</MenuItem>
              ...
              <TableBody>
                {assignments.map((assignment) => (
                  ...
                  >
                    ...
                    <TableCell>
                      <Chip
                        label={getTypeLabel(assignment.type || assignment.Type)}
                        size="small"
                        variant="outlined"
                    ...
                    <TableCell align="center">
                      {(assignment.skillName || assignment.SkillName) ? (
                        <Chip
                          label={assignment.skillName || assignment.SkillName}
                          size="small"
                          color="info"
                          variant="outlined"
                        />
        ...
        <Dialog open={createDialogOpen} onClose={() => setCreateDialogOpen(false)} maxWidth="md" fullWidth>
            ...
            <Grid container spacing={3} sx={{ mt: 0 }}>
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  helperText={formData.type === 'Quiz' ? 'Quiz sẽ tự động tính điểm từ tổng điểm các câu hỏi' : 'Chọn loại phù hợp với mục tiêu học tập'}
                >
                  <MenuItem value="Essay">Bài tập về nhà</MenuItem>
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  fullWidth
                  type="number"
                  label={formData.type === 'Quiz' ? 'Điểm tối đa (tự động)' : 'Điểm tối đa *'}
                  value={formData.maxScore}
                  disabled={formData.type === 'Quiz'}
                  helperText={formData.type === 'Quiz' 
                    ? 'Quiz sẽ tự động tính từ tổng điểm các câu hỏi (bạn thêm câu hỏi ở bước sau)' 
                    : 'Nhập từ 1 đến 10 điểm'}
                  onChange={(e) => setFormData({ ...formData, maxScore: parseInt(e.target.value) || 0 })}
                  inputProps={{ min: 1, max: 10 }}
                  required={formData.type !== 'Quiz'}
                />
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  helperText="Không thể chọn thời gian trong quá khứ"
                  required
                />
              </Grid>
              <Grid item xs={12}>
          ...
          <DialogActions sx={{ px: 3, pb: 2 }}>
            ...
            <Button 
              ...
              variant="contained"
              disabled={!formData.title.trim() || !formData.description.trim() || !formData.skillId || !formData.dueDate || loading}
              startIcon={loading ? <CircularProgress size={20} /> : null}
            >
              {loading ? 'Đang lưu...' : selectedAssignment ? 'Cập nhật' : 'Tạo bài tập'}
            </Button>
          </DialogActions>
        </Dialog>
        ...
        >
          ...
          {menuAssignment?.status === 'Closed' ? (
            ...
            <MenuItem 
              ...
              disabled={menuAssignment?.status?.toLowerCase() === 'draft'}
            >
              <ListItemIcon>
